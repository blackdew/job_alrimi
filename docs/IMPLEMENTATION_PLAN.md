...
### 1.3 데이터 처리 프로세스
1.  **수집 (Collect)**: 주기적(1시간 간격, GitHub Actions)으로 게시판 1페이지 스캔.
...
### 3.1 크롤링 정확성
*   [x] 남해군청 사이트에 새 글 등록 시 10분 이내에 Firestore에 저장되는가?
*   [x] 본문 내 전화번호가 없을 경우 예외 처리가 적절한가? (대표 번호 안내 등)

### 3.2 알림 속도 및 연결
*   [x] 데이터 저장 후 사용자 기기에 푸시 알림이 5초 이내 도착하는가?
*   [x] 알림 클릭 시 해당 상세 페이지로 딥링크(Deep Link) 연결되는가?
*   [x] 상세 페이지에서 전화 버튼 클릭 시 다이얼 화면에 번호가 자동 입력되는가?
